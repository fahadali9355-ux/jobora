    ...
    <div className="min-h-screen bg-[#FBFBF9] flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2">
        ...
        <div className="hidden lg:flex flex-col justify-center px-16 xl:px-24 border-r border-black/5 bg-[#F5F5F2] relative overflow-hidden">
          ...
          <div className="absolute top-20 right-20 w-40 h-40 border border-black/5 rounded-full opacity-20" />
          <div className="relative z-10">
            <span className="text-[10px] uppercase tracking-[0.2em] font-medium opacity-40 mb-8 block">Authentication</span>
            <h1 className="text-6xl xl:text-7xl font-bold tracking-tight mb-6 leading-[0.9]">
              Welcome<br />
              <span className="font-serif italic font-normal text-black/60">Back.</span>
            </h1>
            <p className="text-black/50 text-sm leading-relaxed max-w-sm mb-12">
              Sign in to access your personalized dashboard, track applications, and let AI find your perfect match.
            </p>
            <blockquote className="border-l border-black/20 pl-6 text-sm font-serif italic text-black/40 max-w-xs leading-relaxed">
              "The future of hiring is not searching — it's matching."
            </blockquote>
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center px-8 lg:px-16 xl:px-24 py-12">
          <div className="w-full max-w-md mx-auto">
            <h2 className="lg:hidden text-4xl font-bold tracking-tight mb-2">
              Welcome <span className="font-serif italic font-normal text-black/60">Back.</span>
            </h2>
            <p className="lg:hidden text-black/50 text-sm mb-8">Sign in to your account</p>

            {/* Role tabs */}
            <div className="mb-10">
              <span className="text-[10px] uppercase tracking-[0.2em] font-medium opacity-40 block mb-4">Select Role</span>
              <div className="flex border border-black/10">
                {(['seeker', 'recruiter', 'admin'] as Role[]).map((r) => (
                  <button key={r} onClick={() => setRole(r)}
                    ...
                    {r === 'seeker' ? 'Job Seeker' : r === 'recruiter' ? 'Recruiter' : 'Admin'}
                  </button>
                ))}
              </div>
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="name@company.com"
                    className="w-full pl-12 pr-4 py-4 bg-transparent border border-black/10 text-sm placeholder:text-black/25 focus:border-black/40 focus:outline-none transition-colors" required />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password"
                    className="w-full pl-12 pr-12 py-4 bg-transparent border border-black/10 text-sm placeholder:text-black/25 focus:border-black/40 focus:outline-none transition-colors" required />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-black/30 hover:text-black/60">
                    {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
                <div className="flex justify-end mt-2">
                  <button type="button" className="text-[11px] text-black/40 hover:text-black transition-colors">Forgot password?</button>
                </div>
              </div>
            ...
            <div className="flex items-center gap-4 my-8">
              ...
              <span className="text-[10px] uppercase tracking-widest text-black/30 font-medium">or continue with</span>
              <div className="flex-1 h-[1px] bg-black/10" />
            </div>
            ...
            <p className="text-center mt-10 text-sm text-black/40">
              ...
              <button onClick={() => navigate('/signup')} className="text-[#1A1A1A] font-semibold hover:underline underline-offset-4">Register</button>
            </p>
          </div>
        </div>
      </div>

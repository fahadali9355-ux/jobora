...
const navItems = [
  ...
  { icon: PlusCircle, label: 'Post a Job', active: false },
  { icon: Users, label: 'Candidates', active: false },
  { icon: GitBranch, label: 'ATS Pipeline', active: false },
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-[#1A1A1A] text-white min-h-screen fixed left-0 top-0 z-20">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          {navItems.map(({ icon: Icon, label }) => (
            <button key={label} onClick={() => { setActiveNav(label); if (label === 'Post a Job') navigate('/post-job'); }}
              className={`w-full flex items-center gap-3 px-4 py-3 text-[11px] uppercase tracking-widest font-bold transition-all duration-200 ${activeNav === label ? 'bg-white text-black rounded-sm' : 'text-white/50 hover:text-white/80'}`}>
